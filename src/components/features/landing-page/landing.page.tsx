...
import { CLIENTS, FEATURES, HOW_IT_WORKS, PRICING_CARDS, PRICING_PLANS, USERS } from '@/lib/constant/constants';
...
export default function HomePageComponent() {
  ...
  const handleProPlanClick = async () => {
    ...
    startTransition(async () => {
      // setIsLoading(true);
      const { sessionId } = await postData({ url: '/api/create-checkout-session', data: { price: proPrice } });
    ...
    <div className="relative min-h-screen w-full bg-[#0a0a23] text-white overflow-x-hidden">
      ...
      <section className="relative flex flex-col items-center justify-center py-36 px-4 text-center">
        ...
        <p className="max-w-2xl mx-auto text-2xl mb-12 text-white/80">
          Work together, share instantly, and create beautifully. Powered by Supabase, designed for teams.
        </p>
        <div className="flex gap-6 justify-center">
          ...
          <Link href="#features">
            <Button size="lg" variant="outline" className="border-[#a78bfa] text-[#a78bfa] rounded-full px-10 py-5 text-xl backdrop-blur hover:border-[#38f9d7] hover:text-[#38f9d7]">
              See Features
            </Button>
          </Link>
        </div>
      ...
      <div className="w-full overflow-hidden -mb-1 z-10">
        <svg viewBox="0 0 1440 80" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-20">
          <path fill="#181826" fillOpacity="1" d="M0,32L60,37.3C120,43,240,53,360,58.7C480,64,600,64,720,53.3C840,43,960,21,1080,16C1200,11,1320,21,1380,26.7L1440,32L1440,80L1380,80C1320,80,1200,80,1080,80C960,80,840,80,720,80C600,80,480,80,360,80C240,80,120,80,60,80L0,80Z" />
        </svg>
      </div>

      {/* Features Section */}
      <section id="features" className="py-24 px-4 max-w-6xl mx-auto">
        <h2 className="text-4xl font-bold text-center mb-12 bg-gradient-to-r from-[#38f9d7] to-[#a78bfa] bg-clip-text text-transparent">Features</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10">
          {FEATURES.map((feature) => (
            <div key={feature.title} className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl p-8 flex flex-col items-center text-center border border-[#38f9d7]/40 hover:scale-105 transition-transform">
              ...
              <h3 className="text-xl font-semibold mb-2 bg-gradient-to-r from-[#38f9d7] to-[#a78bfa] bg-clip-text text-transparent">{feature.title}</h3>
      ...
      <section id="resources" className="py-24 px-4 max-w-5xl mx-auto">
        <h2 className="text-4xl font-bold text-center mb-12 bg-gradient-to-r from-[#38f9d7] to-[#a78bfa] bg-clip-text text-transparent">How It Works</h2>
        <div className="flex flex-col md:flex-row gap-10 justify-center">
          {HOW_IT_WORKS.map((step) => (
            <div key={step.step} className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl p-8 flex flex-col items-center text-center border border-[#38f9d7]/40">
              <span className="w-14 h-14 flex items-center justify-center rounded-full bg-gradient-to-br from-[#38f9d7] to-[#a78bfa] text-white text-3xl font-bold mb-4 shadow-lg">{step.step}</span>
              <h4 className="text-lg font-semibold mb-2 bg-gradient-to-r from-[#38f9d7] to-[#a78bfa] bg-clip-text text-transparent">{step.title}</h4>
      ...
      <section className="py-20 px-4 max-w-4xl mx-auto flex flex-col items-center">
        <h2 className="text-3xl font-bold text-center mb-8 bg-gradient-to-r from-[#38f9d7] to-[#a78bfa] bg-clip-text text-transparent">Integrations</h2>
      ...
      <section id="pricing" className="py-24 px-4 max-w-6xl mx-auto bg-white/10 backdrop-blur-xl rounded-3xl my-12 text-white border border-[#38f9d7]/30 shadow-2xl">
        <TitleSection
            ...
            <CustomCard
                ...
                <CardTitle className="text-2xl font-semibold text-white">
                  {card.planType === PRICING_PLANS.proplan && (
                    <>
                      ...
                      <Image src={Diamond} alt="Pro Plan Icon" className="absolute top-6 right-6" width={50} height={50} />
                    </>
                  )}
                  {card.planType}
                </CardTitle>
              ...
              cardContent={
                <CardContent className="p-0">
                  ...
                  {+card.price > 0 ? (
                    <span className="text-[#a78bfa] ml-1">/mo</span>
                  ) : (
                    ''
                  )}
                  <p className="text-white/80">{card.description}</p>
                  {card.planType === PRICING_PLANS.proplan ? <Button
                    variant="secondary"
